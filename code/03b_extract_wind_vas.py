...
#NOTE: the below needs to be previously mounted in terminal with: mount_smbfs //snl/Collaborative/nsrd_climate_impacts/data/raw/wind/ ~/tempmount
    ...
    wind_files_all = sorted(glob.glob(os.path.join(wind_dir, 'vas*nc')))
elif myos == 'Linux':
...
for m in range(len(models)):
    ...
    for s in range(len(sites)):
        ...
        wind_data.to_pickle(os.path.join('03_data', mysite+'_'+mymodel+'_vas.pkl'))
